...
\State $naslednjeStanje[i] \gets$ novoStanje($trenutnoStanje[i], lokalnePtice);$
\EndFor
\State izrisiStanje(naslednjeStanje);
\State $trenutnoStanje \gets naslednjeStanje;$
\EndLoop
\end{algorithmic}
\end{algorithm}
...
\For{$i \gets 0; i < N; i++$}
\State $naslednjeStanje[i] \gets$ novoStanje($trenutnoStanje[i], mrezaPtic);$
\EndFor
\State izrisiStanje(naslednjeStanje);
\State $trenutnoStanje \gets naslednjeStanje;$
\EndLoop
...
\caption{Meritve za OpenMP paralelni algoritem. FPS predstavlja število okvirjev na sekundo, S predstavlja pohitritev, E pa predstavlja učinkovitost.}
\end{table}
...
Meritve izvedene na istem osebnem računalniku kot meritve za prejšne tehnologije so tabelirane v Tabela~\ref{tabela_meritev_mpi}. Uspelo nam je doseči pohitritev, a so bile te najslabše od vseh uporabljenih tehnologij. To pripisujemo predvsem večji komunikaciji saj je to prva tehnologija kjer niti ne komunicirajo prek skupnega pomnilnika. Dodatno se nekaj časa izgubi pri pretvarjanju podatkov. Namreč nemoremo pošiljati tabele struktur $ptica$. Tako moramo vsako ptico pretvoriti v tabelo float števil, ki natančno opišejo stanje ptic (v našem primeru so potrebne štiri float števila). Na strani prejemnika pa je to tabelo števil potrebno pretvoriti nazaj v tabelo struktur $ptica$.


\begin{table}[]
\centering
\label{tabela_meritev_mpi}
\resizebox{10cm}{!}{%
...
\caption{Meritve za MPI paralelni algoritem. FPS predstavlja število okvirjev na sekundo, S predstavlja pohitritev, E pa predstavlja učinkovitost.}
\end{table}

\begin{figure}[t]
\includegraphics[width=\textwidth]{graf_mpi}
\caption{Graf primerja paralelni MPI algoritem z serijskim algoritmom v odvisnosti od velikosti jate.}
\label{img:graf_mpi}
\centering
\end{figure}
